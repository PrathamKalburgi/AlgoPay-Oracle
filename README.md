...
[Live Demo Sandbox](#) · [TestNet Dashboard](#) · [Walkthrough Video](#) · [npm Package](https://www.npmjs.com/package/@algopayoracle/oracle-sdk)
...
Call this from any gateway webhook (UPI, Razorpay, Stripe, PhonePe).
...
To prevent clients from spoofing payment amounts, the system routes checkouts through an authoritative server-side OrderStore architecture:
...
async parseClientPayment({ razorpay_order_id, razorpay_payment_id, razorpay_signature, action }) {
  ...
  // Fetch authoritative cost from order cache
...
## APC-1 Standardized Payment Credentials

Every verified payment produces a portable, self-contained APC-1 payment credential representing an on-chain proof-of-payment that any third-party contract can verify.

**AlgoPay standardizes how real-world payment events become programmable smart contract inputs.**
...
Implementation details optimized for deterministic verification and AVM execution efficiency.

### 1. Opcode Budget Pooling Optimization

AVM opcode limits are bypassed through grouped NOP transaction pooling, enabling efficient on-chain Ed25519 signature verification (`ed25519verify_bare`) without exceeding runtime execution constraints.

The SDK bundles three empty NOP app calls alongside the main `verify_payment()` call inside a single Atomic Transaction Group. This pools the overall transaction execution budget, leaving ample overhead margin after validating the signature.

### 2. Deterministic Byte-Level Encoding

To bypass expensive string manipulation and JSON parsing on-chain, signed payloads are packed into deterministic, packed Big-Endian byte buffers matching this exact binary layout:

```js
// Sourced from src/OracleSigner.js
static buildMessage(canonical_id, action, currency, amount, timestamp, appId = 0) {
  const enc = new TextEncoder();
  const parts = [
    enc.encode("AlgoPay:v1:"),            // Protocol version prefix
    enc.encode(canonical_id),             // Namespaced replay key (provider:payment_id)
    enc.encode(action),                   // Web3 action scoped identifier
    enc.encode(currency.toUpperCase()),   // 3-byte ISO currency standard
    algosdk.encodeUint64(amount),         // 8-byte Big-Endian Unsigned Integer (₹100)
    algosdk.encodeUint64(timestamp),      // 8-byte Big-Endian Unix Timestamp
    algosdk.encodeUint64(appId),          // 8-byte App ID protection
  ];
...
This layout is mapped directly in the Python contract `AlgoPayOracle.py`:
...
### 3. Dual-Purpose Box Storage Pattern

The smart contract uses two box storage topologies to optimize state storage overhead:

- **Oracle Registry Box Keys:** Keyed by the raw 32-byte Ed25519 public key. Value is a simple `b"1"`.
- **Replay Protection Box Keys:** Keyed by the variable length `canonical_id` byte slice. Value is set to `b"1"` on validation.
